...
#       pscc = bytes([0xAA, 0xBB, 0xCC])  
        ...
        # Convertir a entero
        session_nonce_int = int.from_bytes(session_nonce, 'big')

        # Sumar 1
        session_nonce_int += a

        # Convertir de nuevo a bytes (1 byte de longitud)
        session_nonce = session_nonce_int.to_bytes(1, 'big')
        ...
        byte_array[0] = 0x01  # MENSAJE DATOS
        byte_array[1:4] = device_id
        byte_array[4] = session_nonce[0]
        ...
        start_index = 4
        end_index = 50  # Ahora end_index es inclusivo
...
if __name__ == "__main__":
 ...
 for i in range(1800):
    send_message(i % 256)  # Hace que i se mantenga entre 0 y 255
    time.sleep(0.02)  # Espera 1 milisegundo
