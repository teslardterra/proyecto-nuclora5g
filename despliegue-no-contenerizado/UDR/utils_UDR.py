...
def FLA_check_DeviceID(message):
    ...
    conn_db = sqlite3.connect(r'C:\SubscribersLoRa.db', timeout=10)
...
def SLA_retrieve_data(message):
    ...
    cursor.execute("SELECT PSK, SessionDuration FROM SuscribersLoRa WHERE DeviceID=?", (DeviceID,))
    result = cursor.fetchone()
...
def store_transmission(DeviceID, data_type, data):
    ...
    conn_db = sqlite3.connect(r'C:\TransmissionsLoRa.db', timeout=10)
    cursor = conn_db.cursor()
